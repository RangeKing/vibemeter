...
分享与设置作为工具入口。口头禅与洞察卡片并入 VCTI，对比条在 Data。会话回放与数据源暂为过渡路由（侧栏不展示）：会话可从数据页进入，数据源从设置打开。复盘工作区暂不进入 VibeMeter；其旧实现保留在 TokenGraph。`aftervibe` 仅作为旧数据库迁移兼容标识，不再出现在产品界面。
