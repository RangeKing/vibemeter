...
VibeMeter 把实时状态、长期数据和 VCTI 人格装进一款本机优先的 macOS 应用。Claude Code、Codex 与 DeepSeek Harness 正在思考、读取、调用工具，还是等你处理，Notch 会及时告诉你；会话、Token、成本、模型、工具与 Skill 使用留在本机，逐渐形成可回看、可分享的数据。证据足够后，24 种 VCTI 人格会给你一份真正属于自己的协作画像。

它不接管 Agent，不替你批准权限，也不改写源码仓库。读不到的数据会明确标注，不用漂亮的 `0` 掩盖缺失。

## 0.6.0：边缘侧栏与毛玻璃

在「设置 → 边缘侧栏」开启独立的屏幕边缘面板，可选择左侧或右侧，也适用于没有硬件 Notch 的显示器。悬停展开，移开后延迟收起；固定按钮保持展开，Escape 收起。侧栏为每个受支持的 Agent 画一个环，默认只显示本机检测到、且有额度可读的 Agent，可在「设置 → 边缘侧栏」逐项调整（没有额度的也能勾选显示）。显示订阅的剩余额度与重置时间：环形读数取该来源最紧的用量窗口，卡片按窗口列出剩余比例和重置时间。没有可读订阅的 Agent 照样有环，但明确标注读不到，不以 0 补齐；实时会话仍在 Notch 显示。

订阅额度之外，DeepSeek 与 Moonshot（Kimi）支持读取 **API 密钥余额**。在「设置 → API 账户」添加密钥即可，同一来源可以添加多个：卡片按账户分别列出余额、赠送与充值，并给出合计；余额没有分母也不会重置，所以不画进度条、不显示重置倒计时。密钥存入 macOS 登录钥匙串，仅在查询余额时读取，不写入数据库、日志或导出。端点与响应结构参考 [CodexBar](https://github.com/steipete/codexbar)，相关 MIT 声明保留于 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。现有顶部 Notch 保留。
...
DeepSeek Harness 不需要安装 Hook。VibeMeter 只读观察其本机会话事件流，提供与 Codex 同级的精确实时生命周期、历史分析、轨迹回放、来源状态与跳回入口，并且不会修改 Harness 配置或会话。

Notch 空闲时完全缩回实体刘海；有活动时，单会话在紧凑左翼显示来源图标与项目名，多会话才显示按来源统计的会话数；右翼只显示优先级最高的一项状态。点击或在实体刘海/两翼停留约 300 ms 都会展开；悬停展开后移开约 500 ms 自动收起，点击其他窗口也会收起。当前展开可临时固定，手动关闭或重启后恢复自动收起。Notch 与菜单栏可以分别关闭。不具备实体 Notch 的屏幕会诚实降级为菜单栏与主界面。Claude Code、Codex、DeepSeek Harness、Kimi Code 与 ZCode 提供精确实时生命周期；其中 DeepSeek Harness、Kimi Code 与 ZCode 只读观察本机结构化状态。Cursor、OpenClaw 与 Hermes 仅按本机可读取能力进入历史数据、回放和 VCTI。
