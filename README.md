...
在「设置 → 边缘侧栏」开启独立的屏幕边缘面板，可选择左侧或右侧，也适用于没有硬件 Notch 的显示器。悬停展开，移开后延迟收起；固定按钮保持展开，Escape 收起。侧栏显示各家订阅的剩余额度与重置时间：环形读数取该来源最紧的用量窗口，卡片按窗口列出剩余比例和重置时间。来源未返回额度时明确标注，不以 0 补齐；实时会话仍在 Notch 显示。现有顶部 Notch 保留。
