...
- Schema 29: adds local project metadata and user-managed project groups without changing source repositories.
- Schema 30: adds `execution_relations`, canonical relation-evidence links, indexed root/parent/child/status access, reverse evidence lookup, and bounded affected-branch counts for takeover-only Delegation Attention.
- Schema 31: adds `memory_accesses`, canonical memory-access evidence links, indexed session/source-session/operation/active access, and reverse evidence lookup. The projection stores activity metadata only, never memory content.
- Parser 6.11.0: scans complete local user/agent text, filters code, paths, secrets, and punctuation-only noise, then retains only ranked derived phrase counts without storing source text.
...
Schemas 20–31 use the same staged-copy migration and rollback boundary as schema 19. A database is installed only after the latest schema version, quick/integrity checks, foreign-key checks, and required-column checks pass; an interrupted migration keeps the previous database and its validated rollback artifact available for recovery.
